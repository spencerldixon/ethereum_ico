  ...
  it('handles delegated transfers', function() {
    return Token.deployed()
      .then(function(instance) {
        ...
        // Send some tokens from our admin account to account 2 to test with
      ...
      .catch(function(error) {
        ...
        return tokenInstance.transferFrom(fromAccount, toAccount, '15', {
          from: spendingAccount,
        });
      })
      .then(assert.fail)
      .catch(function(error) {
        assert(
          error.message.indexOf('revert') >= 0,
          'cannot transfer more than approved amount',
        );
        return tokenInstance.transferFrom.call(fromAccount, toAccount, '5', {
          from: spendingAccount,
        });
      })
      .then(function(success) {
      ...
      .then(function(balance) {
        assert.equal(
          balance.toNumber(),
          90,
          'deducts balance from transferring account',
        );
      ...
      .then(function(balance) {
        assert.equal(
          balance.toNumber(),
          10,
          'adds balance to receiving account',
        );
      ...
      .then(function(allowance) {
        assert.equal(
          allowance.toNumber(),
          0,
          'deducts transferred tokens from allowance',
        );
      });
  });
